...
import { isAdminAuthorizedAsync, auditJwksFailure } from '../lib/adminAuth'
const jwks = require('../lib/jwks')

afterEach(() => { jest.restoreAllMocks(); delete process.env.ADMIN_JWKS_URI })

test('isAdminAuthorizedAsync returns false when JWKS fetch fails and logs via audit helper', async () => {
  ...
  // confirm helper logged the structured marker
  expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('AUDIT_JWKS_FAILURE'), expect.anything(), expect.anything())
})
...
test('isAdminAuthorizedAsync returns false when token is expired (verify throws) and logs via audit helper', async () => {
  process.env.ADMIN_JWKS_URI = 'https://example.com/.well-known/jwks.json'
  jest.spyOn(jwt, 'decode').mockReturnValue({ header: { kid: 'k1' } } as any)
